...
# Added validation logging function
...
def main():
    ...
    train_dataset = IXIDataset(args.dataset, mode='train', debug_mode=args.debug_mode)
    valid_dataset = IXIDataset(args.dataset, mode='valid', debug_mode=args.debug_mode)    
    ...
    model = HFGAN(dim=args.ch_dim, num_inputs=3, num_outputs=1, dim_mults=(1,2,4,8,10), n_layers=args.numlayers, skip=True, blocks=False,image_size=256,H=256,W=256,batch_size=batch_size)
            ...
            with torch.no_grad():
                ...
                if accelerator.is_main_process:
                    ...
                    if cur_valid_con_loss < min_valid_con_loss:
                        ...
                        accelerator.save_state(output_dir=cur_best_model_path) 
                        logger.info(f"save model to {cur_best_model_path}")
